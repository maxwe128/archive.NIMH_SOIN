codedWater_onCon1000<-Calculate_GraphTheory(dataList = "/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/onOffGroupList.txt",nEdgesOrThreshold = getEdgesFromPercent(10:50,116),analysisType = "group",comparison = T,threshType = "positive",edgeType = "partial",structuralData = T,numberPermutations = 10000,brainWide = T)
reformPermData<-listToMatrixByRow(codedWater_onCon1000[[7]])
reformDiffBW<-metaListRearrange(codedWater_onCon1000[[5]])
...
abindGroupDiff<-abind(codedWater_onCon1000[[4]],along = 3)
...
groupDiffBW<-codedWater_onCon1000[[4]]
...
cbind(highestBetweeness,highestCloseness,highestDegree,highestEigenvector,highestTransitivity)
...
saveRDS(permTestNewBWneg,"/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/onConPermTestBWneg")
saveRDS(avgSMRG2M,"/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/conAvg")
conHubs<-zSMRG2M[,1:4][apply(X = zSMRG2M[,1:4]>2,MARGIN = 1,FUN = sum)>.99,]
conHubAreas<-rownames(conHubs)
saveRDS(conHubAreas,"/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/conHubAreas")
