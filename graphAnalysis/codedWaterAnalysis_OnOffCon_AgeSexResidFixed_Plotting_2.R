require(ggplot2)
OnGroupData<-readRDS("/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/codedWater_onConResidAgeSexFixed10000_On_2")
ConGroupData<-readRDS("/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/codedWater_onConResidAgeSexFixed10000_Con_2")
OffGroupData<-readRDS("/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/codedWater_onOffResidAgeSexFixed10000_Off_2")
onOffContrast<-readRDS("/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/codedWater_onOffResidAgeSexFixed10000_2")
onOffGraphData<-onOffContrast[[8]]
remove(onOffContrast)
onConContrast<-readRDS("/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/codedWater_onConResidAgeSexFixed10000_2")
onConGraphData<-onConContrast[[8]]
remove(onConContrast)
OffConContrast<-readRDS("/x/wmn18/elliottml/GraphTheoryAnalyses/codedWaterAnalysis/savedObjects/codedWater_offConResidAgeSexFixed10000_2")
OffConGraphData<-OffConContrast[[8]]
remove(OffConContrast)

###Control Hubs analysis###
controlsHubs<-onConGraphData[[2]]$grp2[,3][onConGraphData[[2]]$grp2[,3]>2]
...
#####ON OFF
onOffContrastMatrixForPlot<-as.data.frame(cbind(c(onOffContrastPosMeanValues),rep(colnames(onOffContrastPosMeanValues),each = 3),rep(groupOrder,13)))
colnames(onOffContrastMatrixForPlot)<-c("betweeness","area","group")
...
onOffContrastMatrixForPlot<-as.data.frame(cbind(c(onOffContrastNegMeanValues),rep(colnames(onOffContrastNegMeanValues),each = 3),rep(groupOrder,11)))
colnames(onOffContrastMatrixForPlot)<-c("betweeness","area","group")
...
#####ON CON
onConContrastMatrixForPlot<-as.data.frame(cbind(c(onConContrastPosMeanValues),rep(colnames(onConContrastPosMeanValues),each = 3),rep(groupOrder,8)))
...
onConContrastMatrixForPlot<-as.data.frame(cbind(c(onConContrastNegMeanValues),rep(colnames(onConContrastNegMeanValues),each = 3),rep(groupOrder,11)))
colnames(onConContrastMatrixForPlot)<-c("betweeness","area","group")
